...
# Title for the app (moved before model loading for better UX)
st.markdown(
...
# Expecting 'my_model.pkl' to be in the same directory as the app.py script
...
try:
    ...
    if isinstance(model_package, dict):
        ...
        model_type = model_package.get('model_type', 'Decision Tree')
        threshold = model_package.get('threshold', 0.5)
        feature_names = model_package.get('feature_names', [])
        scaler = model_package.get('scaler', None)
    else:
        # Direct model format (just the sklearn model)
        model = model_package
        ...
        st.warning(f"⚠️ Model loaded without metadata. Using defaults: threshold={threshold}")

except FileNotFoundError:
    st.error(f"❌ Model file 'my_model.pkl' not found.")
    st.info("Please ensure 'my_model.pkl' is in the same directory as the app.py script.")
    st.stop()
except Exception as e:
    st.error(f"❌ Error loading model: {str(e)}")
    st.info(f"Attempted to load 'my_model.pkl'. Please check the file's integrity.")
    st.stop()
...
with tab1:
    ...
    with col1:
        ...
        # FICO Score Group (not used in model but shown for reference)
...
with tab3:
    ...
    if st.button("🔮 Predict Approval Likelihood", type="primary", use_container_width=True):
        ...
        try:
            ...
            # If we have feature names from the model, use them
            if feature_names:
